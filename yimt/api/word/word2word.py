...
from yimt.api.word.utils import (
    download_or_load, download_os2018, get_savedir
)
from yimt.api.word.tokenization import (
    get_sents, get_vocab, update_dicts
)
from yimt.api.word.methods import (
...
class Word2word:
    ...
    def make(
        ...
        t0 = time()
        print("Step 2. Constructing and tokenizing sentences")
        sents1 = get_sents(
            lang1_file, lang1, cased, n_lines, num_workers
        )
        sents2 = get_sents(
            lang2_file, lang2, cased, n_lines, num_workers
        )
        ...
        # word <-> index
        word2x, x2word, x2cnt = get_vocab(sents1)  # sorted by word frequency

...
from yimt.core.utils import compat, misc
...
def get_dataset_size(dataset, batch_size=5000):
    ...
    Example:
      ...
      >>> yimt.data.get_dataset_size(dataset).numpy()
      5
...
def filter_irregular_batches(multiple):
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.filter_irregular_batches(3))
...
def filter_examples_by_length(
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.filter_examples_by_length(...))
...
def random_shard(shard_size, dataset_size):
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.random_shard(2, 6))
...
def shuffle_dataset(buffer_size, shuffle_shards=True, dataset_size=None):
    ...
    Example:
      ...
      >>> dataset = tf.data.Dataset.range(6)
      >>> dataset = dataset.apply(yimt.data.shuffle_dataset(3))
      >>> list(dataset.as_numpy_iterator())
      [2, 3, 1, 0, 4, 5]
...
def batch_dataset(batch_size, padded_shapes=None):
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.batch_dataset(...))
...
def batch_sequence_dataset(
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.batch_sequence_dataset(...))
...
def training_pipeline(
    ...
    * prefetching
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.training_pipeline(...))
...
def inference_pipeline(
    ...
    Example:
      ...
      >>> dataset = dataset.apply(yimt.data.inference_pipeline(...))
